...
import static org.avphs.traksim.trakSimFiles.MyMath.*;
...
public class HandyOps { // first some useful debug logging ops..
    ...
    public static double SafeParseFlt(String aStr) {
        double valu = Fix2flt(SafeParseInt(NthItemOf(false, 1, aStr)), 0),
                base = 1.0;
        ...
        try {
            if (fpart != 0) {
                ...
                valu = valu + Fix2flt(fpart, 0) / base;
            }
        } //~if //~try
        catch (Exception ex) {
        }
    ...
    public static void WriteTiff32Image(String filename,
        ...
        try {
            while (true) {
                ...
                lxx = (iMax(nby, pixels.length) + base) << 2;
                nby = nby << 2;
                data = new byte[lxx];
                why++; // why = 2
                if (data == null) break;
