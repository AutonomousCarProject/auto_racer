...
package org.avphs.sbcio;   // 2018 February 10
...
public class ArduinoIO implements PWMController { // Adapted to Java from arduino.cs ... (FakeFirmata)
    ...
    private static int StartUpTimeMS = (int) System.currentTimeMillis();
    protected int[] digitalOutputData;
    protected SerialPortSwapper surrealPort;
    ...
    public void digitalRead() {
        try {
            while (this.surrealPort.getInputBufferBytesCount() >= 3) {
                ...
                if (msg[0] == 100 && IOSettings.readMessages) {
    ...
    private void sendArduinoSignal(int messageType, int pin, int angle) {
        byte[] msg = new byte[3];
