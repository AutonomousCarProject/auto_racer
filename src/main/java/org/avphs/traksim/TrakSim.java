 ...
 */
package org.avphs.traksim; // (class TrakSim)                  // 2019 July 25
...
public class TrakSim {
     ...
     */
    public int[] GetPiDigs() {return PiDigits;} //~GetPiDigs
     ...
     */
    public int[] GetMyScreenAry() {return myScreen;} // so to trade out multiple..
     ...
     */
    public int GetMyScreenDims() {return (SceneTall<<16)+SceneWide;}
     ...
     */
    public int GetImgWide() {return ImageWide;}
     ...
     * @return       = Dzoom
     */
    public double DenomZoom() {return Dzoom;}
     ...
     */
    public boolean IsItProxim() {return IsProxim;}
    ...
    public int Map2screen(double Vat, double Hat, double engle, int pie,
        ...
        while (true) {
            ...
            if (!keep) {
                ...
                WallAim = ftmp;} //~if // >0, c-wise frm N
            ...
            if (nuly) { // 1st time this wall seen, cache some data..
                ...
                else CashDx = fdst*sinB/sinA;} //~if // = dist in m to: wall hits cntr, >0
            ...
            if (aim != 0.0) {
                ...
                xtmp = MyMath.Cose;
                if (false) ftmp = ftmp*xtmp;} //~if // shortens too much // cx = pix H-posn..
            ...
            if (CashDx>0.5) { // not too close to pixelate (CashDx>0.4 is safe@52mm)..
                ...
                ppm = (int)Math.round(ftmp/CashDx);} //~if // = scr.pix*256/m, always >0
            ...
            if (CashDx>0.0) if (engle != 0.0) if (aim != 0.0) if (xtmp<1.0) {
                ...
                if (nx>0) if (nx<65536) ppm = (nx<<16)+ppm;} //~if // (engle != 0.0)
     ...
     */
    public boolean IsFixtSped() {return SimSpedFixt;}
     ...
     */
    public int LensFocus() {return ZoomFocus;}
            ...
            System.out.println(HandyOps.Flt2Log("(LinSlope) "+msg,Hoffm,
                    HandyOps.Flt2Log(" *",Voffm,HandyOps.Flt2Log(" (",Vat,
                            HandyOps.Flt2Log("/",Hat,HandyOps.Flt2Log(" ",Vstp,
                                    HandyOps.Flt2Log("/",Hstp,")"))))) ));} //~LineSlope
    ...
    public static int EncoPavEdge(int tall, int wide, int recell, int why) {
        ...
        if ((MyMath.iAbs(tall)>MyMath.iAbs(wide)) || (why<0)) { // more N-S than E-W,
            ...
            Kconst = Mconst*(recell>>16)+MyMath.SgnExt(recell);} //~if // in 25cm units
            ...
            Kconst = Mconst*MyMath.SgnExt(recell)+(recell>>16);} //~else
        eps = Cast_F2I(Mconst);
                    ...
                    HandyOps.Flt2Log(HandyOps.IffyStr(EW," EW Kc="," NS Kc="),Kconst,
                            HandyOps.Flt2Log(" Mc=",Mconst,HandyOps.Dec2Log(" ",tall,
                                    HandyOps.Dec2Log("/",wide,HandyOps.Int2Log(HandyOps.IffyStr(grtr,
                                            " > "," < "),recell,HandyOps.Dec2Log(" ",why,""))))) )));} //~if
        return bitz;} //~EncoPavEdge
    ...
    public static void WhiteAlfa(int tall, int wide, int[] theImgs) {
        ...
        if (Mini_Log) if (NoisyMap) if (Qlog<0)
            System.out.println(HandyOps.Dec2Log("  <WhitAlf> ",tall,
                    HandyOps.Dec2Log("/",wide,HandyOps.ArrayDumpLine(
                            theImgs,
                            20,11))));
        for (rx=tall-1; rx >= 0; rx += -1) for (cx=wide-1; cx >= 0; cx += -1) { // top-to-bottom..
        ...
        while (uppy !=0) {
            ...
            uppy = 0;
            thar = tops+wide;
            for (rx=tall-1; rx >= 0; rx += -1) for (cx=wide-1; cx >= 0; cx += -1) { // then bottom-up..
                here--;
                if (here >= theImgs.length) break;
                info = theImgs[here];
                thar--;
                if (info != Transprnt) { // magical interior transparent =0xFEFEFE
                    ...
                    else if (theImgs[thar]<0) info = -1;} //~if
                else info = -1;
                if (info<0) {
                    theImgs[here] = info;
                    seen = true;
            ...
            thar = -1-wide;
            uppy = 0;
            for (rx=tall-1; rx >= 0; rx += -1) for (cx=wide-1; cx >= 0; cx += -1) { // then top-down again..
                here++;
                if (here >= theImgs.length) break;
                info = theImgs[here];
                thar++;
                ...
                else seen = false;}} //~while               // Mini_Log=F..
        if (Mini_Log) if (NoisyMap) if (Qlog<0)
    ...
    private static void HeadLiSho(int why, double deep,
        ...
        if (whom==0) if (why>0) {
            ...
            SeenHedLite = true;} //~if   // see also: (HedShoLi)
        if (Qlog<0) System.out.println(HandyOps.Int2Log("(HedLiSho) ",why,
                HandyOps.Int2Log("/",whom,HandyOps.Flt2Log(" ",deep,
                        HandyOps.Flt2Log(" @",Vat,HandyOps.Flt2Log("/",Hat," --"
                                + HandyOps.ArrayDumpLine(ShoHeadLite,0,40))))) ));} //~HeadLiSho
            ...
            for (yx=tall; yx >= 0; yx += -1) {
                ...
                for (nx=wide; nx >= 0; nx += -1) {
                    ...
                    if (abit<0) abit = 0x40000000;
                        ...
                        abit = 0x80000000;
                        whar++;} //~if
                    else abit = abit>>1;}} //~for
            why = 0; // success!
            break;} //~while               // Mini_Log=F..
                                    ...
                                    HandyOps.Dec2Log(" => ",img&0xFFFFFF,HandyOps.Dec2Log("+",img>>24,
                                            HandyOps.Int2Log("/",info,HandyOps.Int2Log("/",more,
                                                    HandyOps.Dec2Log(" @ ",here,
                                                            HandyOps.Dec2Log(" = ",why,aLine))))) ))))) ));} //~if   // why =
        return PaintIx;} //~Ad2PntMap
        ...
        if (Log_Draw || NoisyMap) if (Qlog<0) if (deep==0) if (!seen)
            TempStr = TempStr + HandyOps.Dec2Log(" ",rx,HandyOps.Dec2Log("/",cx,
                    HandyOps.Colo2Log(" = ",colo,HandyOps.Hex2Log(" / x",info,8,
                            HandyOps.Dec2Log(" @ ",thar,"")))));} //~MapBucket
    ...
    private static int Luminator(char xCh, double Vat, double Hat,
        ...
        while (true) { // once thru..
            ...
            if (theLum==null) {
                ...
                if (colo !=0) break;} //~if // 0-brite lumin needs no LumMap
            ...
            if (preSc) { // build display data..
                ...
                theList = HandyOps.RepNthLine(aWord,lino,theList);} //~if
            more = 0;
            ...
            if (nxt>99) { // nxt is altitude in park meters
                ...
                if (colo>zx*8) {
                    ...
                    if (colo>zx*16) break;}} //~if // too bright (wash out center)
            ...
            if (aim>0) { // we have directionality/shadows.......................... +
                ...
                if (zx>0) {
                    ...
                    else if (wix >= 250.0) wix = 310.0;} //~if // ignore infinite radius
                if (wix<256.0) {
                    if (wix==0.0) { // 0 counts as effectively infinite: max actual..
                        ...
                        far = 63.0;} //~if // soft-edge threshold
                    else {
                ...
                if (NoisyMap) if (MapLogged) { // if (Qlog..) -- included in M_L
                    ...
                    if (more<180) {
                        if (ShoHedLit) { // ShoHedLit=true
                            ...
                            else HeadLiSho(rpt,deep,Vat,Hat);} //~if // always logs
            ...
            for (rpt=MapTall-1; rpt >= 0; rpt += -1) for (kx=MapWide-1; kx >= 0; kx += -1) { // in pk meters
                ...
                if (aim>0) while (true) { // test if this point in shadow..
                    ...
                    if ((whom&2)==0) { // upper or right 45-quadrant, <0 is OK
                        if (NoisyMap) aStr = "^";
                        whar = -whar;} //~if // now >0 means (V)west/(H)north (OK)
                ...
                while (true) { // set theLum for this loc..
                    ...
                    break;} //~while // (set theLum)         // NoisyMap=F..
                if (NoisyMap || MapLogged) if (xcell<0) // if (Qlog..) -- in ML
                    System.out.println(xStr);} //~for // (rpt,kx)(calc whole lum map) // "  ... "
            LumiLogg();
            rpt = -1; // no error
            break;} //~while // (once thru)
        TempStr = theList;
        return rpt;} //~Luminator
    ...
    private static int BackImage(char xCh, int rx, int cx,
        ...
        if (MapLogged) {
            logy++; // logy = 1
            if (Qlog>0x10000) whoz = Qlog>>16;} //~if
        ...
        while (true) { // uses NuData,Wally,nBax to preserve state across calls
            InWalls = false; if (SeenWall == null) break; // errn = -120;
            ...
            if ((xCh == 'v')||(xCh == 'h')||(xCh == 'p')) { // prescan plain wall..
                ...
                if (nx<256) { // capture this line for log..
                    ...
                    if (xCh == 'p') {
                        Save4Log = HandyOps.RepNthLine("-" + aLine,nx+1,Save4Log);
                        nx = nx+2;
                        Save4Log = HandyOps.RepNthLine("+" + aLine,nx,Save4Log);}} //~if
                why = 0;
                nBax = nx;
                doing = nBax;
                curvd = false;
                errn = -1; // no error
                break;} //~if // (prescan 'v'/'h'/'p')
            ...
            if (xCh != 'V') if (xCh != 'H') if (xCh != 'P') { // not plain walls..
                if (xCh >= 'c') { // if (xCh <= 'm') { // prescan, extract r/c..
                    ...
                    frnt = (rx<<16)+cx;} //~if // (prescan, extract r/c)
                if (kx>0) { // got ending r/c spec.. // kx=NthOffset(0,"/",xStr)
                    ...
                    if (frnt != rocox) { // find angle (can't from singleton)
                        ...
                        if (engle <= 0) engle = engle+360;}} //~if // (find angle) (got ending)
                else if (xCh >= 'c') { // if (xCh <= 'm') {
                    engle = HandyOps.SafeParseInt(xStr); // dir'n
                    while (engle <= 0) engle = engle+360;} //~if
                ...
                if (engle>0) { // else don't have a valid angle
                    ...
                    if (rocox==0) if (xCh == 'g') if (aCh == ' ') { // calc end frm len..
                        ...
                        if (pie <= 0) { // no lxx given, try for wall width..
                            ...
                            if (pie>0) pie = nx/pie;} //~if // im.wi/ppm
                        if (pie>1) {
                    ...
                    if (xCh == 'm') { // manual corner has 8 different aims, no pref..
                        ...
                        if ((aim&1) !=0) { // got a diagonal..
                            ...
                            else kx = nx+8;} //~if // 8=45 up diag, 9=135 dn, 10/11 west'ly
                ...
                if (rocox==0) if (xCh >= 'c') { // if (xCh <= 'g') // assume singleton..
                    ...
                    why = why+1;}} //~if // why = 1..7, depending on source of rocox
            ...
            if (xCh == 'm') { // prescan (all params known/assumed valid)..
                ...
                if (nx >=0) if (nx<256) { // insert face/corner singleton.. // **
                    ...
                    xStr = "";
                    curvd = true;
                    nBax = nx;} //~if // (insert corner singleton)
                zx = 0;
                why = 0;
                doing = nBax;
                errn = -1; // no error
            ...
            if (xCh == 'g') { // prescan..
                ...
                if (aCh >= '0') { // we have an explicit aim..
                    ...
                    if (engle<0) {
                        if (aCh == 'm') { // (engle<0 shouldn't be possible)
                            ...
                            else info = aim*90-45;} //~if // if (aim<8) if (aim >= 4)
                        else if (aim<5) engle = aim*90;} //~if
                    if (engle==0) engle = 360;} //~if
                if (aCh == 'M') aCh = 'm';
                ...
                if (NuData>0) { // aCh = HandyOps.CharAt(1,aLine);
                    ...
                    if (engle >= 0) {
                        Darken = engle;
                        NuData = kx;} //~if
                    else NuData = NuData&-0x01000000|kx&0xFFFFFF;} //~if
                else if (engle >= 0) {
                    Darken = engle;
                    NuData = kx;} //~if
                zx = Wally;
                ...
                if (aim>3) { // aim: 4=NE,5=SE,6=SW,7=NW (corner); 8=45 up diag, 9=135 d
                    ...
                    if (aim>15) aim = (aim>>4)&3;} //~if           // right side of..
                ...
                if (nx>0) if (nx<256) {
                    ...
                    SeenWall[nx+768] = (-lino)&0xFFFF|skip;} //~if // line+ for rt link
            ...
            if (xCh >= 'c') { // if (xCh <= 'd') { // prescan & fill in curve.. // **
                ...
                if (skip+5>sang) {
                    sang = 90;
                    skip = 0;} //~if // ** shortened curve ends not yet used **
                kx = 0;
                ...
                aWord = " 0" + aWord;
                errn = -1; // no error
                for (kx=kx; kx <= 999; kx++) if (kx>0) if (kx<ImWalCurves.length) { // no cont
                    info = ImWalCurves[kx];
                    nuly = (info>>4)&15;
                    offx = info&15;
                    more = info>>8;
                    ...
                    if (info<0) { // aim: 4=NE,5=SE,6=SW,7=NW (corner); 8=45 up diag..
                        ...
                        if (tall>7) { // M.. // tall: +8 = '.', +4 = L/R, +2 = L, +1 > 45..
                            ...
                            zx = (zx<<4)+nxt+8;} //~if // = +8..11; angle = (zx&3)*90+45
                        else zx = zx*17;
                        nxt = 0;
                        aStr = HandyOps.Dec2Log(" ",wide*45,aStr) + aWord;
                        if (wide==0) wide = 8; // (360)
                        zx = wide*64+zx;} //~if // 'm' corners are always *45 (often != more)
                    else aStr =
            ...
            if (xCh == ' ') { // build/vfy preliminary data in SeenWall.....(BackImag)
                ...
                for (skip=1; skip >= 0; skip += -1) for (base=1; base <= nBax; base++) { // define segments..
                    if (base<0) break;
                    if (base>255) break;
                    frnt = SeenWall[base];
                    if (frnt==0) continue;
                    ...
                    for (nx=1; nx <= nBax; nx++) { // find all walls in this segment (same anchor)..
                        ...
                        if (prio>0) if (prio<256) {
                            ...
                            if ((px&-0x2000)==0) // not already linked nor has pie info..
                                SeenWall[prio+256] = (nx<<16)|px|0x2000;} //~if
                        SeenWall[nx] = here|0x20000000;
                        SeenWall[nx+768] = SeenWall[nx+768]&-0x10000|base;
                        prio = nx;}} //~for // (nx) (base) (skip) // (define segments)
                // why = 0;
                ...
                if (false)
                    for (base=1; base <= nBax; base++) { // verify corners come just before seg heads..
                        ...
                        if ((info&0x1000) !=0) { // ignore curve segments..
                            px = 0;
                            continue;} //~if
                        zx = 0; ix1 = 0; ix2 = 0; ix3 = 0; // ix0 = 0; // (for log)
                        frnt = SeenWall[base];
                        ...
                        if (Mini_Log) if (MapLogged || ((Qlog&256) !=0))
                            if (why<99) if ((why<8)||(zx !=0)) {
                                if (more<0) {
                                    lino = -(more>>16);
                                    aLine = HandyOps.NthItemOf(true,lino,theList);} //~if
                                else aLine = "";
                        ...
                        errn = ~errn; // errn = -123
                        why = (errn<<16)+base;
                        break;} //~for // (base) (verify corners)
                why = 0;
                if (errn<0) break; // errn = -123
                errn++; // errn = 123 (not an error) // omit 'vfy preliminary' part 3?..
                if (false)
                    for (skip=1; skip >= 0; skip += -1) for (base=1; base <= nBax; base++) { // link segments into chains..
                        if (base <= 0) break;
                        if (base>255) break;
                        frnt = SeenWall[base];
                        if (skip==0) {
                            if ((frnt&0x40000000)==0) continue;} //~if // do heads in 2nd pass
                ...
                errn = -1; // no error
                // doing = nBax;
                break;} //~if // (build/vfy preliminary data)
            ...
            if (whom) { // (xCh == 'G') // otherwise 'V'/'H'/'P'
                ...
                if (curvd) { // can't happen (not called)
                    ...
                    if (((pie&-512)==0) && (pie <= 360)) { // pie is credible view angle
                        ...
                        if (haff>0) { // haff is view pie half-width..
                            ...
                            else if (centrd) pie = pie|512;} //~if // centered flag => 0x800000
                        else pie = 0;} //~if
                    else pie = 0;
                    if (pie>0) {
                        pie = pie<<2;
                        ...
                        else if (zCh == 'R') pie++;} //~if // prefer right fase => 0x1000
                    ...
                    if (zCh == 'C') if (aCh > ' ') { // if (pie>0) // (already known)
                        ...
                        if (((nxt&255)==0)||((nxt&0xFF0000)==0)) // .. same V is H-line
                            pie = pie|1024;} //~if // end-cap flag => 0x400000
                    nxt = info;
                    ...
                    if (WideRatio<0) {
                        ix1 = ix1|0x200;
                        WideRatio = WideRatio&0xFFFFFF;} //~if
                    ix2 = (ppm<<24)+offx;
                    ix3 = 0;
                    if (didit) { // "*": recycle image (lxx=0 OK)..
                        ix1 = ix1|0x80000000;
                        why = why+2;} //~if     // xStr = HandyOps.NthItemOf(false,10,aLine);
                    else if (HandyOps.NthOffset(0,"!",xStr) >= 0) {
                        if (false) if (lxx>0.0) {
                            ftmp = MyMath.fMax(MyMath.Fix2flt(wide,0)/lxx,1.0);
                            ix3 = MyMath.iMin(MyMath.Trunc8(ftmp),63);} //~if // (obsolete)
                        ix3 = 0x80000000;} //~if // H-ppm to stretch // was: = ((nx|128)<<24)+ix3;
                                                                        ...
                                                                        HandyOps.Int2Log(" => ",ix1,HandyOps.Int2Log(" ",ix2,
                                                                                HandyOps.Int2Log(" ",ix3,HandyOps.IffyStr((ix1&0x200)==0,"",
                                                                                        HandyOps.Dec2Log(" ",WideRatio,HandyOps.Fixt8th("=",
                                                                                                WideRatio>>13,""))))) ))))) ))))) )));
                    nxt = 0;}} //~if // ('G' straight)(whom)   (in BackImag)
                ...
                Hat = MyMath.Fix2flt(cx,0);
                curvd = false;
                doit = false;} //~else // (xCh != 'G') (else 'V'/'H'/'P')     (in BackImag)
                    ...
                    if (Mini_Log) if (MapLogged || ((Qlog&256) !=0))
                        System.out.println(HandyOps.Dec2Log("+-+ ",doing,
                                HandyOps.Int2Log(" = ",info,
                                        HandyOps.IffyStr(doit,Save4Log,aWord))));} //~if
                if (Mini_Log) if (MapLogged || ((Qlog&256) !=0)) {
                    aStr = "/";
                    aWord = "\n      @@@ ";
            ...
            for (nx=0; nx <= 9999; nx++) { // lay down this wall in its grid locns................
                ...
                if (errn !=0) if (aCh != 'm') if (thar >= 0) if (thar<myMap.length) {
                    ...
                    if (prio != 0) doit = false;} //~if // early bird gets the worm
                ...
                if (doit) { // T: poke this cell..
                    if (prio==0) if (whar >= 0) if (whar<myMap.length) {
                        ...
                        myMap[thar] = zx;
                        myMap[whar] = info;
                        if (aCh == 'm') break;} //~if // always stop after 1st poke of singletn
                    doit = false;} //~if
            ...
            nxt = 0;
            errn = -1; // no error
            break;} //~while     // why=0 always except 'G' and vfy prelim (' ')..
        ...
        if (MapLogged) if ((errn <= 0)||Mini_Log || ((Qlog&256) !=0)) {
            if (aLine != "") {                      // Mini_Log=T
                ...
                if (didit) aLine = aLine + "\n      ==> '"
                        + HandyOps.NthItemOf(true,lino,theList) + "'";} //~if
            else aLine = " _''";
            ...
            for (nx=0; nx <= doing+8; nx++) {
                if (nx>255) break; if (SeenWall == null) break;
        ...
        while (true) { // 12*brk, all errs, most logged here:
            ...
            for (rpt=99; rpt >= 0; rpt += -1) { // 5*brk, (one at end), 1*cont (same as BackIm)
                ...
                if (prio>0) if (prio<256) { // interpolate partial segment..
                    ...
                    tall = tall-(((tall*prio+127)>>8)&-8);} //~if
                if (whom) wide = -wide;
    ...
    private static int[] Mappy0(String theList, String aLine,
        ...
        if (yx !=0) while (true) { // we have managed art or paint..
            ...
            if (yx>0) {
                ...
                PaintIx = 3;} //~if // init'ly just the terminator
            ...
            if (yx>0) { // this line at end of data blocks seeing ".StopInfo" beyond..
                ...
                didit = true;} //~if // T: caller gets to use shortened file
                ...
                switch (xCh) {
                    ...
                    case 'J': // else if (xCh == 'J') {
                        ...
                        if (yx>0) if (zx>4) if (zx<16) {
                            ...
                            if ((sofar&zx)==0) sofar = sofar+zx+1;
                            theText = theText + "\n" + xStr;
                            nCells = nCells+yx;} //~if // word 5 is number of animation frames
                        break;
            ...
            if (nBax>0) {
                ...
                yx = nBax*4+yx;
                nBax = MyMath.iMin(nBax,255)+kx;} //~if // = (->1st,+BGs) used by BackImag
            ...
            if (myMap != null) {
                ...
                if (EWx) LuminanceMap = new int[MapAsize+2];} //~if
            ...
            if (ImDims>0) { // here = myMap.length-2 = thar+theImgs.length
                ...
                ImSz = 0;} //~else
            rez = true;
            break;} //~while // (we may have managed art)
    ...
    public static int[] BuildMap(String theList, int ImDims,
        ...
        while (true) {
            ...
            if (myMap==null) {
                why = 8;
                myMap = new int[nuIxBase+GridSz+4];} //~if
            why++; // why = 1/9
            if (myMap==null) break;
            here = nuIxBase;
            nuIxBase++;
            if (here>0) if (here<myMap.length-1) {
                ...
                myMap[here] = 0;} //~if // visual start of map in log
        ...
        if (why>0) {
            ...
            if (why !=8) return null;} //~if // last early exit
        ...
        if (PaintIx>0) {
            ...
            if (nBax>0) wait = xcell-(xcell&-xcell)+wait;} //~if
        if (myMap != null) if (myMap.length>12) { // (can't fail, got it above)
            ...
            myMap[ArtBase+2] = wait;} //~if // =0 if no paint, else offset to paint map
        SameData = 0;
        ...
        for (lino=2; lino <= 999; lino++) {
            ...
            if (xCh == '.') { try {
                ...
                if (!fini) {
                    ...
                    if (zx>0) {
                        ...
                        sofar = HandyOps.SafeParseInt(HandyOps.NthItemOf(false,2,xStr));} //~if
                    else fini = true;} //~if
                if (fini) {
                    aWord = "' (Imgs)\n  =-=";
                    ...
                    aWord = aWord + HandyOps.ArrayDumpLine(myMap,nImgs+7,5)
                            + "\n  _:_" + HandyOps.ArrayDumpLine(AnimInfo,0,5);} //~if
                else if (Mini_Log && NoisyMap) {
                    zx = HandyOps.Countem("\n",xStr);
                    aWord = "'\n --" + HandyOps.ArrayDumpLine(myMap,36,4)
                            + "\n  _:_" + HandyOps.ArrayDumpLine(AnimInfo,0,5)
                            + HandyOps.Dec2Log(" -- next do (",zx,").. {") + xStr + "}";} //~if
                else aWord = "'";
                aWord = " '" + aLine + aWord;
                if (Qlog<0)
            ...
            } catch (Exception ex) {here = -lino; break;}
                continue;} //~if // (xCh == '.')
            ...
            switch (xCh) { // convert gentle curve specs to straight line segments..
                ...
                case 'W': // westward   // leaves prio inside (pos'bly -1/8 from) new face
                    ...
                    for (zx=0; zx <= yx; zx++) {
                        ...
                        else if (EWx) {if ((here&0xFFFF)==0) nxt++;} //~if // major dim'n hit..
                        ...
                        if (wait==0) if (prev !=0) if (recell !=0) {
                            ...
                            if ((rx&-0x80000) !=0) if ((cx&0x3FF8) !=0) {
                                ...
                                info = recell&0xFFFF|prev&-0x10000;} //~if
                            else if (EWx) {
                                rx = recell&0x70000;
                                if (rx>0x40000) info = recell+0x80000;
                                else if (rx<0x40000) info = recell-0x80000;} //~if
                            else if ((recell&4)==0) info = recell-8;
                            else if ((recell&3) !=0) info = recell+8;} //~if
                        if ((recell !=0) || NoisyMap || MapLogged) for (kx=-1; kx <= 1; kx++) {
                            ...
                            if ((recell&-0x10000)==0) break;} //~for // (kx)
                        recell = xcell; // this is 1st hit in this next cell
                        if (nxt<0) break;} //~for // (zx)
                ...
                case 'U': // painted (line) spec..
                    ...
                    if (thar>0) {
                        ...
                        if ((tall==0)||(wide==0)) {
                            ...
                            if ((nxt&1) !=0) { // flip H/V..
                                ...
                                tall = wide;
                                wide = zx;}} //~if
                        xStr = HandyOps.NthItemOf(false,4,xStr);} //~if
                    thar = HandyOps.SafeParseInt(xStr);
                    kx = HandyOps.NthOffset(0,"^",xStr);
                    rpt = -88;
                    if (tall <= 0) break;
                    ...
                    if (yx<HalfMap) {
                        ...
                        rpt--; // rpt = -92
                        if (PaintIx==zx) break;} //~if // something went wrong in Ad2LnMp
                ...
                case 'T': // start up RealTime Base..
                    ...
                    if (frax) { // allow for fractional position, but add test bits..
                        ...
                        aWord = aWord + HandyOps.Fixt8th(" ",cx,aStr);} //~if
                ...
                case 'J': // macro to build animation sequence..
                    ...
                    if (tmp>16) {
                        ...
                        aStr = HandyOps.Substring(0,tmp,aLine);} //~if
                    else {
                        xStr = " --";
                        aStr = aLine;} //~else
                    rpt--; // rpt = -28
                    ...
                    if (false) {
                        ...
                        if (zx>0) aim = HandyOps.SafeParseInt(HandyOps.RestOf(zx+1,aStr));} //~if
                    else aim = 0;
                    Hinc = 1.0;
                    deep = 0.0;
                ...
                case 'Y': // macro to insert artifact from standard descriptor
                    ...
                    if ((kx<4)||(kx>15)) {
                        ...
                        else yx = nxt-270;} //~if    // (preserve fractional coordinates)..
                    aWord = "\nO " + HandyOps.NthItemOf(false,2,aLine) + " "
                    ...
                    if (kx==0) aWord = aLine // stop sign..
                        ...
                        for (zx=1; zx <= 99; zx++) { // add all linked lines..
                            ...
                            if (yx>0) { // got a view angle offset..
                                ...
                                if (yx >= 360) break;
                                yx = yx+nxt;
                                while (yx >= 360) yx = yx-360;} //~if
                            else yx = nxt;
                            xStr = xStr + HandyOps.Dec2Log(aWord,yx," ") + aStr;} //~for
                        aWord = xStr;} //~if
                        ...
                        if (nxt >= 0) {
                            ...
                            xStr = HandyOps.Substring(0,nxt+1,aLine);} //~if
                        else xStr = aLine;
                        ...
                        aWord = aLine
                                ...
                                + aWord + " 0 " + HandyOps.NthItemOf(true,10,theText);} //~if
                    else kx = -1;
                    if (kx >= 0) {
                        theList = HandyOps.RepNthLine(aWord,lino,theList);
                        if (MapLogged) if (Qlog<0) System.out.println(HandyOps.Dec2Log("  [$] ",rx,
                ...
                case 'O': // artifact info (see ShoArtif) (not postponed)..
                    ...
                    if (kx<0) kx = 0;
                        ...
                        if (thar>0) if (thar<myMap.length) {
                            ...
                            myMap[thar] = ((sofar-3)<<16)+(tmp&0xFFFF);} //~if // (in case 'O')
                        nxt = 0;} //~if
                    yx = (tall<<16)+65536-yx;
                ...
                case 'Q': // convert track edge to grass (no border yet)
                    ...
                    if (myMap==null) break;
                    thar = myMap.length-1;
                    for (thar=thar; thar >= nuIxBase; thar += -1) if (myMap != null) if (thar>0)
                        if (thar<myMap.length) {
                            nxt = myMap[thar];
                            if (nxt >= 0) if (nxt != 0x44444444) continue;
                            myMap[thar] = 0x66666666;} //~if
                    dont = -1;
                    wait = 0;
                    prev = 0;
                    prio = 0;
            ...
            for (rpt=rpt; rpt >= 0; rpt += -1) {
                ...
                if (false) if (recell !=0) if (here<myMap.length-(HalfMap+2)) {
                    ...
                    myMap[here+HalfMap+1] = colo;} //~if
                here = here+nxt;} //~for // (rpt)
            here = 0;} //~for // (lino)
        prio = nuIxBase;
        here = 0;
        nxt = 0;
    ...
    public int ZoomMap2true(boolean aim2, int rx, int cx) { // rtns (r,c)<<6
        ...
        if (aim2) while (true) { //    Hat = MyMath.Fix2flt(MyMath.SgnExt(rez),6);
            ...
            AverageAim = Facing;
            // NuData++; // caller does
            break;} //~while   // ZMD=12,16 ZSf=4 ZB=34,322 ZSc=32. ZW=0.1 ZT=10,8 // *
        if (Mini_Log) if (Qlog<0)
    ...
    public int ZoomMapCoord(boolean yx, double Vat, double Hat) {
        ...
        while (true) {
            ...
            if (!yx) { // yx=false converts image coordinate '(ZoMaCo) F' ..........
                ...
                cx = (int)Math.round(Hat*deep);} //~else
    ...
    public void PokePixel(int colo, int rx, int cx) { // -> myScreen
        ...
        if (didit==0) here = rx*WinWi+cx;
        if (myPix != null) if (here<myPix.length)
            if (here >= 0) myPix[here] = colo;} //~PokePixel
    ...
    public void RectFill(int colo, int rx, int cx, int rz, int cz) { // BR not sz
        ...
        if (rz<rx) {here = rz; rz = rx; rx = here;}
        if (cz<cx) {here = cz; cz = cx; cx = here;}
        if (rx<0) rx = 0;
    ...
    public void DrawRedX() { // to show it crashed
        ...
        for (rx=0; rx <= 4; rx++) { // draw red (crashed) "X" in lower left corner..
            ...
            PokePixel(0xFF0000,ImHi-8+rx,8-rx);} //~for
        if (Qlog==0) return;
    ...
    public void DrawSteerWheel(int posn, boolean scaled, boolean dash2) {
        ...
        if (DrawDash>0) if (dash2) { // show dashboard..
            ...
            if (Log_Draw) myDash = " '" + LefDash + "/" + myDash + "'";} //~if // (dash2)
        if (scaled) if (posn<127) { // scale it to TapedWheel..
            ...
            if (info !=0) for (nx=(TapedWheel[0]&255)-1; nx >= 0; nx += -1) {
                ...
                break;} //~for // always exits here, with info set
            posn = info;} //~if // (scaled)
        for (doit=0; doit <= 1; doit++) for (here=doit; here <= 11; here++) { // draw steering wheel..
            thar = TapedWheel[here&15];
            whar = thar>>16;
            info = SteerColo; // SteerColo=0xCC9900;
            if (doit==0) {
                if (here==posn) info = 0xFFFFFF; // PixWhit;
                whar = whar+SteerMid;} //~if
            else {
                if (here+posn ==0) info = 0xFFFFFF; // PixWhit;
                whar = SteerMid-1-whar;} //~else
            if (Log_Draw) myDash = "";
            for (thar=thar&255; thar <= 99; thar++) {
                ...
                for (cx=0; cx <= 31; cx++) {
                    ...
                    bitz = bitz>>1;
                    if (bitz==0) break;} //~for // (cx)
                if (doit==0) whar++;
                else whar--;}}} //~DrawSteerWheel
    ...
    public void SimStep(int mode) {
        ...
        if (doit) {
            ...
            StepOne = (nuly==1);
            NextFrUpdate = 0;} //~if // in mode=2 this is when to update image
    ...
    private void HeadShoLines() { // only from BF
        ...
        if (!SeenHedLite) ShoHeadLite[0] = 0;} //~HeadShoLines // final log
    ...
    private int ViewAngle(int whar, int locn, int info, boolean prox) {
        ...
        if (Vx<0) { // (un)lit luminary..
            ...
            else Vx = 60;} //~if // otherwise assume 60 degrees
        ...
        return res;} //~ViewAngle // rtn >0 if in view
    ...
    private int Animatron(int whar, int locn, boolean logy) { // for animated art
        ...
        if (anim>0) while (true) {
            ...
            if (anim>4) { // one-shot indexed movable stuff (setup @ [%]f)..
                ...
                while (true) { // fetch img ref(w/locn) from anim'n list by frame +..
                    ...
                    break;} //~while
                break;} //~if // (one-shot indexed movable stuff)
                ...
                if (zx>3) {
                    ...
                    if (zx>7) { // bits: +8=(V>)
                        if (info>0) seen = zx-16;} //~if
                    else if (info<0) seen = zx-16;} //~if // bits: +4=(V<)
                zx = zx&3;
                if (zx>0) {
                    ...
                    if (zx<0) { // if both H&V are tested, both must fire..
                        if (seen<4) seen = seen|-16;} //~if
                    else if (seen<0) seen = seen&15;} //~if // only V fired, so untrigger
                if (zx<0) {
                    zx = TimeBaseSeq>>16;
                ...
                break;} //~if // why = 34/35 // (start up RealTime Base)
                ...
                if (locn>0) {
                    ...
                    if (kx>0) if (kx<theInx.length) {
                        ...
                        if (info>0) if ((info&-0x10000000)==0x10000000)
                            TmpI = (kx<<16)+kx+4;} //~if
                    if (TmpI==0) locn = locn-0x80000000;}} //~if // (traffic light)
        ...
        if (skipper) if (TmpI==0) for (nx=9; nx >= 0; nx += -1) {
            ...
            if (whar<theInx.length) if (((theInx[whar]-seen)&0x0FFFFFFF)==0)
                continue;
            TmpI = (whar<<16)+whar;
            break;} //~for                // GoodLog = true, logy = Fax_Log = T..
    ...
    private void ShoArtifax() { // sort by distance, then display far-to-near..
        ...
        while (true) {
            ...
            if (RealTimeBase !=0) if (doit) { // RTN in seconds/8..
                ...
                else RealTimeNow = HandyOps.TimeSecs(true)/125-RealTimeBase;} //~if
            ...
            for (nx=NumFax-1; nx >= 0; nx += -1) { // calc in-view, distance
                ...
                if (info<0) locn = locn&0x0FFFFFFF; // ceiling light
                    ...
                    if (TmpI>0) {
                        ...
                        here = TmpI>>16;}} //~if // here -> actual data
                why = 8;
                    ...
                    if (aim>VuEdge) dx = -8; // not in view
                        ...
                        if (true)
                            if ((aim>4.0) || (aim+4.0<0.0)) {
                                ...
                                step = step*MyMath.Cose;} //~if // so same pix row as if center-view
                        cx = (int)Math.round(ftmp+fImMid);
                        if (cx<0) {
                            cx = 0;
                            dx--;} //~if // dx = -1 (don't show)
                        else dx = (int)Math.round(step);} //~if // (incl 4 bits frac'l pk m)
                    else dx = -8;
                    if (lino>myFax.length-2) break; // (can't)
                    myFax[lino] = here;
                    myFax[lino+1] = (dx<<16)+cx; // dx in 6cm units, cx in pixels
                    lino = lino+2;} //~if // (duplicate locn = alt view)
            ...
            if (whar>0) for (nx=whar; nx >= 2; nx += -2) for (lino=2; lino <= nx; lino += 2) { // sort..............
                if (lino<2) break;
                if (lino>myFax.length-2) break; // (can't)
                Vx = myFax[lino-1];
                Hx = myFax[lino+1];
            ...
            if (Mini_Log) if (logy) if ((Qlog&32) !=0) {
                ...
                System.out.println(aLine + " => "
                        + HandyOps.ArrayDumpLine(myFax,whar+2,8));} //~if
            why++; // why = 13
            ...
            for (nx=whar; nx >= 0; nx += -2) { // draw them (no cont, 3brk @ front, -1brk @ end)..
                ...
                if (whar>0) if (whar<theInx.length-8) {
                    ...
                    tall = theInx[whar+3];} //~if // image H & W
                if (Mini_Log) {
                ...
                if (whar>0) { //                                             (ShoArtifx)
                    ...
                    if (tall>0) { // calc artifact drawing spex..
                        ...
                        if (kx<0) { // -kx is implied file pix below img to grnd (else kx=0)
                            ...
                            kx = (int)Math.round(MyMath.Fix2flt(kx,0)/step);}} //~if // kx: scrn px
                    ...
                    if (tall>0) { // draw this artifact (unless oops>0)..
                        ...
                        for (zx=zx+zx; zx >= 0; zx += -1) { // outer loop is horizontal..
                            ...
                            if (cx >= ImWi) oops = oops|256; // off right end;  off left end..
                                ...
                                for (yx=cx; yx <= LayerSz-1; yx += ImWi) {
                                    ...
                                    if ((wx&-0x100000)==rotst) // at same distance..
                                        if (tole>0) if (WalzTall != null) {
                                            ...
                                            if (tole>(wx&0xFFFF)) wx = wx|0x80000;} //~if // art is taller
                                            ...
                                            TempStr = TempStr + HandyOps.Dec2Log(" Lyr=",yx/ImWi,
                                                    HandyOps.Dec2Log(" +",cx,HandyOps.Dec2Log(" [",yx,
                                                            HandyOps.Hex2Log("] ",rotst,8,HandyOps.Int2Log(":",wx,
                                                                    HandyOps.Dec2Log(" ",info,""))))) );} //~if
                                        if (topper==0) topper = info;
                                        else if (topper<info) topper = info;
                                        continue;} //~if // saved wall top overrules WalzTall
                                ...
                                if (topper<0) if (oops<0x10000) { // art behind/below wall?
                                    ...
                                    if (Mini_Log) if (TempStr != "") TempStr =
                                            TempStr + HandyOps.Dec2Log(" => ",topper,"");}} //~if
                            if ((oops==0)||Mini_Log&&logy) for (dx=0; dx <= tall+12; dx++) {
                                ...
                                while (Vat >= 1.0) {
                                    Vx = Vx-ImgWi;
                                    Vat = Vat-1.0;}} //~for // dx (this column up)
                            if ((oops&0xC0FFF) !=0) break;
                            oops = 0;}} //~if //~for zx (outer loop is horizontal) (tall>0)
                                    ...
                                    HandyOps.Dec2Log(" *",ppm,HandyOps.Dec2Log(" @ ",Pmap,
                                            HandyOps.Dec2Log(" ",ImgWi,HandyOps.Dec2Log(" ",tops,
                                                    HandyOps.Flt2Log(" st=",step*16.0,HandyOps.IffyStr(oops==0,"/16",
                                                            HandyOps.Hex2Log(" x",oops,4,""))))) )))));} //~if
                if (Mini_Log) {
                    if (aLine != "") if ((Qlog&32) !=0)
                        if (looky==0) System.out.println(aLine); // "  (::) "
                    TempStr = "";} //~if
                if (oops >= 0x80000) break; // all the rest are out of view
                if (false) if (OpMode==3) break;} //~for // nx (draw them)
            if (why>13) break;
            why = 0;
            break;} //~while              // HandyOps.IffyStr(!logy,"",   // why =
                                            ...
                                            HandyOps.Flt2Log(" ",fudge,HandyOps.Dec2Log(" o",OpMode,
                                                    HandyOps.TF2Log("/",StepOne,HandyOps.Fixt8th(" t=",RealTimeNow,
                                                            HandyOps.Dec2Log("=",FakeRealTime,HandyOps.Fixt8th("-",FakeTimeBase,
                                                                    HandyOps.Flt2Log(" ",fox,""))))) ))))) )))));} //~ShoArtifax
    ...
    private int TossLayer(int bakk, int info, int inv, int skey, int cx,
        ...
        int whar = MaxLayers+1, whom = bakk, ix0, ix1, ix2, tall = 0, thar = 0,
                why = 0;
        if (WaLayerz==null) whar--; else try {
        ...
        } catch (Exception ex) {whar = MaxLayers+3;}
        return whar;} //~TossLayer // whar = 0..MaxLayers+3|0x10000
    ...
    private int SortLayer(boolean ImgWall, int info, int cx, int inv,
        ...
        try {
            if (Mini_Log) if ((Qlog&256) !=0)
                if ((logy>0)||(TripLine<0)||(doing==whoz)) {
                    if ((cx&15)==0) logz = true;
                        ...
                        logz = true;} //~if
                    else if (ix0+1<0) if (ix0+cx==0) logz = true;} //~if
            ...
            while (true) {
                ...
                for (nx=cx; nx <= cx+ImWi*(MaxLayers-1); nx += ImWi) { // copy SeenWal -> WaLayer..
                    ...
                    if (ix0>0) { // closer (larger) row + (=rx) counts higher..
                        ...
                        if (ix1<0x3000) // otherwise: both plain walls & near-edge
                            ...
                            skey = ((0xFFF&~ix1)<<8)+skey;} //~if // rrrhhh++
                    ...
                    deep++;} //~for // (nx: copy SeenWal -> WaLayer)
                if (!ImgWall) { // plain wall goes in layer 0..
                    ...
                    if (deep>0) { // deep is the number of prior layers
                        if ((whom&0xFFF) >= 4080) { // existing plain wall..
                            ...
                            nx++;} //~if // new replaces prior layer 0, but check the rest
                        else if (rx>whom) nx++; // ditto (new plain wall is in front)
                        else nx--;} //~if // push layer 0 up
                    why++; // why = 5
                    whar = 0;
                ...
                for (nx=deep; nx >= thar; nx += -1) { // find where to insert new..
                    ...
                    Shifty = true;} //~for // T: layer(s) mod'd, copy WaLayerz -> SeenWall
                if (whar >= MaxLayers) why = why+2; // why = (x3)/x5
                break;} } catch (Exception ex) {whar = MaxLayers+3;}
        if (whar >= 0) if (whar<MaxLayers) { // put item into its SeenWal layer..
            ...
            if (logz||Shifty) { // only for log..
                ...
                WaLayerz[whar+MaxLayers*2] = skey;} //~if
            ix0 = whar*ImWi+cx;
                        ...
                        HandyOps.Dec2Log(" ",cx,HandyOps.Int2Log(": ",info,
                                HandyOps.Int2Log(" [",inv,HandyOps.Dec2Log("] => ",whar,
                                        HandyOps.Int2Log(": ",ix0,HandyOps.Int2Log(" ",ix1,
                                                HandyOps.IffyStr(ix1<0x30000000,""," (.??.)"))))) ))));} //~if
        return (xx<<MxLayShf)+whar;} //~SortLayer // whar = 0..MaxLayers+3=7
    ...
    private void DoWalls(boolean doit) { // no early rtn "{DW} "           // nBax
        ...
        while (true) { // once through, precalc & render exit separately..
            ...
            if (doit) { // render the precalc'd image walls (no early exits)..........
                if (ShowMap) if (BreadCrumbs != null) if (nCrumbs<HalfLoaf)
                    for (doing=BreadCrumbs[Crummy]; doing <= Crummy-1; doing++) if (doing>0)
                        if (doing<Crummy) { // Crummy = 255, HalfLoaf = Crummy/2-3
                            ...
                            if (info>0) if (yx>0) { // ..also in close-up map..
                                ...
                                DrawLine(ink,info,yx,zx,kx);} //~if // (also in close-up)
                            if (Mini_Log) if (logy !=0) { // if (Qlog..) -- included in M_L
                                System.out.println(TempStr);                    // "   '' "
                                TempStr = "";} //~if
                            yx = 0;} //~if // (ShowMap)
                if (xx>0) { // = TmpI = rx for 1st ink on floor
                    ...
                    else xx = (xx<<20)+5;} //~if // default to BackWall, farthest floor
                why = 15;
                if (Mini_Log) if ((Qlog&256) !=0) {
                    ...
                    for (cx=0; cx <= ImWi-1; cx++) { // 2 brk (err), 3 cont (when done)
                        ...
                        if (ipx==0) {
                            if (offx >= 4080) {
                                ...
                                if (whom>0) if (rx>fz) if (WalzTall != null) {
                                    ...
                                    if ((tall&0x20000) !=0) fz = rx;} //~if // T: close to park edge
                                if (topper>0) if (topper<ImWi*8) {
                                    SeenWall[topper] = -1;
                                    topper = 0;} //~if
                                orient = BackWall;
                                gotit = true;} //~if // (insert backwall behind)
                            if (fz>ImHi-DrawDash) fz = ImHi-DrawDash;} //~if // (ipx=0)
                        if (whoz>0) { // whoz = Qlog>>16;
                            ...
                            if (looky==0) {
                                ...
                                if (whom==whoz) logy++; // logy = 2;
                                    ...
                                    else if (zx+1<0) if (zx+cx==0) logy++;} //~if
                                if (logy>0) {logy++; }}} //~if
                        if (info==0) {if (!gotit) continue;} //~if
                        ...
                        if (whom>0) { // (ImgWall=T)
                            ...
                            for (rx=rx; rx >= 0; rx += -1) {
                                ...
                                else offx = offx-zx*ImgWi;} //~for // (rx)
                            if (topper>0) if (topper<ImWi*8) {
                                SeenWall[topper] = ~fz;
                                topper = 0;}} //~if // (topper>0) (ImgWall=T)
                            ...
                            if (topper>0) if (topper<ImWi*8) {
                                SeenWall[topper] = -1;
                                topper = 0;} //~if
                            PokePixel(orient,rx,cx);}}} //~for // (rx) (cx) (ipx: 4 layers)
                why = -2;
                ...
                if (ShoNos) { // display img wall +s on-scrn..
                    ...
                    for (nx=0; nx <= nBax+1; nx++) {
                        ...
                        LabelScene(HandyOps.Dec2Log("",nx,""),24,zx,0xFFFFFF);} //~for // (nx)
                    if (Mini_Log) {
            ...
            if (HandyOps.NthOffset(0,"\n\n",Save4Log) >= 0) {
                ...
                while (true) { // remove unneeded blank lines from log info
                    ...
                    zx = Save4Log.length();
                    if (zx==xx) break;
                    xx = zx;}} //~if //~while // (remove unneeded blank lines)
            ...
            for (doing=1; doing <= 255; doing++) { // each wall.. (6 brk, 23 cont)            // (doing)
                ...
                if (ImgWall) {
                    ...
                    if (topper>0) { // (calc now for log)..
                        ...
                        topper = (int)Math.round(MyMath.fMax(Vtmp*1024.0,0.0));}} //~if // pkmm
                else Tally = 0;
                ...
                if (Mini_Log) if ((Qlog&256) !=0)
                    if ((logy>0)||(TripLine<0)) { // if (Qlog..) -- included in M_L
                        ...
                        if (zx==0) { // ** this is leftover info to log from prev doing **..
                            ...
                            else if (roff != 0.0) zx++;} //~if // = rt end face length reduction
                        if (TmpI !=0) TempStr =
                        ...
                        sawn = true;
                        TempStr = "";} //~if // (Mini_Log)                         // ink = 0;
                TmpI = 0;
                whom = 0; ibase = 0; yx = 0; Hx = 0; Hz = 0;  roff = 0.0; // (for log)
                Vbase = 0.0; Hbase = 0.0; Vstp = 0.0; Hstp = 0.0; ftmp = 0.0;
                Hpm = 0.0; fase = 0.0; fz = 0; pz = 0;  EWx = false; rpt = false;
                ...
                if ((ix1&0x400000) !=0) { // this is an end-cap, are we between sides?
                    ...
                    if (pie>0) if (aim>0) if (engle<60.0) if (engle+60.0>0) {
                        ...
                        if (false) { // NG: car can be distant but parallel **
                            ...
                            if (xx<pie) if (xx+pie>0) pie = 0;}} //~if // good enough, use it
                ...
                if (ibase<0) if (ink<0) { // both ends not visible, maybe can't show..
                    ...
                    whom--; // whom = -5
                    if (ink+1==0) continue;} //~if // 5th cont of 23
                    ...
                    if (ink <= 0) { // binary search for far visible end..
                        ...
                        while (true) { // (far binary search loop)
                            ...
                            if (WallAim != 0.0) {
                                ...
                                else if (fase != WallAim) fase = (fase+WallAim)*0.5;} //~if
                            zx = TmpI;
                            pz = ink&0xFFFF;
                            gotit = (pz==ImWi-1);
                            if (!gotit) if (ink>0) {
                            ...
                            Hbtm = Hinc;
                            zx = logy*5;} //~while // (far binary search loop)
                        if (zx==0) ink = 0; // zx = scr.pix*256/m (=0: off-scrn)
                        if (ink <= 0) break;} //~if // whom = -9 // failed (far binary search)
                    whom--; // whom = -10
                    if (ibase <= 0) { // binary search for near visible end..
                        ...
                        while (true) { // (near binary search loop)
                            ...
                            if (WallAim != 0.0) {
                                ...
                                else if (lift != WallAim) lift = (lift+WallAim)*0.5;} //~if
                            kx = TmpI;
                            pz = ibase&0xFFFF;
                            gotit = (pz==0);
                            if (!gotit) if (ibase>0) {
                            ...
                            Htop = Hinc;
                            kx = logy*9;} //~while // (near binary search loop)
                        if (kx==0) ibase = 0;} //~if // failed (near binary search)
                    break;} //~while // (exit when both ends visible)
                pz = 0; // whom = -8/-9/-10
                ...
                if (pie>0) { // pie = (ix1>>24)&0x7F = half-width of view wedge
                    ...
                    pz = 0;
                    fz = 0;} //~if // then or otherwise it's in view..
                ...
                if (ImgWall) { // image wall..                               // (DoWall)
                    ...
                    if (engle != 0.0) if (!gotit) { // don't oblique-compress centered
                        ...
                        else if (frnt) Hpm = 0;} //~if // ditto if WideRatio is active
                    if (Hpm==0) { // need to know step rate..
                        ...
                        if (Hpm>88.0) if (fase>0.0) { // if (near) parallel, use fase..
                            ...
                            Hpm = MyMath.fAbs(fase);} //~if // (near parallel)
                        // whom--; // whom = -17
                        ...
                        if (Hpm<0.15) {
                            ...
                            if (fz>16) if (Hpm<0.01) continue;} //~if // +24 of 24
                        Hpm = ppm/Hpm;} //~if // Hpm>0 always (need to know step rate)
                    if (frnt)   // WideRatio = H-ppm*256/V-ppm..
                        Hpm = MyMath.Fix2flt(WideRatio<<2,10)*Hpm;} //~if // (ImgWall)
                ...
                if (ImgWall) { // image wall..  // kx = scr.pix*256/m (frm Map2scr)..
                    ...
                    if (zx != kx) {
                        ...
                        rx = MyMath.iAbs(rx);
                        if (rx<333) rx = 0;} //~if // (math is wrong, so unused)
                ...
                if (ImgWall) { // image wall.. (ppm=(image ppm)*256, pct=left trim in m)
                    ...
                    if (gotit) { // centered..
                        ...
                        if (Mini_Log) if (looky==0) TempStr = HandyOps.Dec2Log(" C:",xx,
                                HandyOps.Dec2Log(" *",stip,"")) + TempStr;
                        if (xx>0) offx = offx+xx;} //~if // can't <0 (centered)
                        ...
                        if (Mini_Log) if (looky==0) TempStr = HandyOps.Dec2Log(" AA:",nx,
                                HandyOps.Dec2Log(" => ",offx,"")) + TempStr;} //~if
                    kx = kx-fz;} //~if // (ImgWall) (image wall)
                ...
                for (cx=Hx; cx <= Hz; cx++) { // each visible H-pix (1*brk, 4*cont after log)..
                    ...
                    if (looky==0) if (xx>0) if (Mini_Log) if ((Qlog&256) !=0) {
                        ...
                        if (xx>0) { // show details for this column..
                            if (!sawn) {
                                ...
                                if (xx>0) {
                                    ...
                                    if (RtnStr != "") {
                                        ...
                                        System.out.println(RtnStr);}} //~if // (didn't see doing line)
                                xx = 1;
                                if (sawn) xx = 6;
                                sawn = false;} //~if
                            ...
                            RtnStr = "";
                            maxz++;}} //~if // (show details) (logy>0||-TripLine=cx)
                    TmpI = 0; // (done)
                    ...
                    offx = offx+xx;} //~for // (cx: each visible H-pix)
                kx = logz&0xFFF;
                zx = (logz>>16)-kx;
                if (zx>0) {
                    zx = zx+4;
                    kx = Math.max(kx-2,0); }
                logz = 0;
                if (looky==0) if (whoz>0) logz = ImWi;} //~for // (doing: each wall)
            if (GroundsColors<0) if (SeenWall[LayerSz]==0) SeenWall[LayerSz]++;
            why = 0; // (find all visible walls)
            break;} //~while // (precalc) (once through, precalc & render)
        if (Mini_Log) if ((Qlog&0x900) !=0) { // Qlog&2048|Qlog&256
            ...
            if (looky==0) {                  // see "<RowCeil>" in log..
                ...
                else TempStr = TempStr + Save4Log;} //~if
    ...
    public int MapColor(int optn, double Vat, double Hat, double ink) {
        ...
        while (true) { // once thru (exit when done)..
            ...
            if (tkonly) { // tkonly = (optn&8)
                ...
                break;} //~if // why = +6/14/.. // all others default =0
            if (info==0) { // outside the park..
                ...
                why++; // why = +8
                info = 5;} //~if // indoors, so put BackWall there (outside the park)
            ...
            if (!flat) if (RectMap>0) while (true) { // we have paint (see Ad2PntMap)
                ...
                while (true) { // look for paint that covers Vat,Hat (3x cont)..
                    ...
                    if ((more&0x04000000) !=0) { // hi-res (32ppm) image..
                        ...
                        Vx = (MyMath.Trunc8(Hat*32.0)&7)+(Vx<<3)+nx;} //~if // so 8x better res
                    ...
                    kolo = 0; // enable luminance
                    why = why|64;
                    break;} //~while // found paint (look for paint that covers)
                break;} //~while // (we have paint)
            ...
            if (info<0) { // pavement edge crosses this square.................. track
                ...
                if ((info&0x20000000) ==0) { // H-line..
                    ...
                    if ((info&0x40000000) ==0) { // > is in
                        ...
                        else if (deep >= Kconst) colo = WhitLnColo;} //~if // white line
                    else if (deep+ledge<Kconst) colo = kolo; // gray
                    else if (deep <= Kconst) colo = WhitLnColo;} //~if // white
                else {
                    ...
                    else if (deep <= Kconst) colo = WhitLnColo;} //~else
                if (ledge==0.0) if (colo==WhitLnColo) colo = kolo; // (info<0)
                kolo = 0;} //~if // enable luminance
            ...
            if (FloorDims>0) while (true) { // ............................. floor tile
                ...
                if (px<0) if (CheckerBd>0) if (((rx^cx)&CheckerBd) !=0) {
                    ...
                    else if (colo==GrasColo) colo = GrasDk;}} //~if
            why++; // why = 8/9/10
            break;} //~while // (once thru)
                                                                        ...
                                                                        HandyOps.Flt2Log("/",ink,HandyOps.Int2Log(HandyOps.IffyStr((colo<0)
                                                                                ...
                                                                                HandyOps.Int2Log(") = ",why,                                  // why =
                                                                                        HandyOps.TF2Log(" c2=",car2,HandyOps.IffyStr(kolo==0," *lum",
                                                                                                ""))))) ))))) ))))) )))) ;
                TempStr = "";}} //~if // (Qlog&16)
        return colo;} //~MapColor // TmpI = Pmap = cell+
    ...
    public void SeeOnScrnPaint(int rx, int cx, int tall, int wide, int here,
        ...
        for (lino=0; lino <= tall; lino++) {
            ...
            for (posn=0; posn <= wide; posn++) {
                ...
                if (info<0) {
                    if (colo<0) continue;
                    info = colo;} //~if
                PokePixel(info,rx+lino,cx+posn);}} //~for
        if (colo<0) return;
        if (rx==0) return;
        ...
        DrawLine(MarinBlue,rx,cx+wide+1,rx+tall,cx+wide+1);} //~SeeOnScrnPaint
        ...
        try {
            ...
            while (true) { // (once through, exit early if error)
                ...
                if (ShowMap) if (DoCloseUp) if (ZooMapDim !=0) while (true) {
                    ...
                    switch (kx&15) {
                        case 0: case 1: case 15: // facing north..
                        ...
                        case 2: // facing north-east..
                            ...
                            break;
                        case 3: case 4: case 5: // facing east..
                            rx = Vmap-(yx>>1);
                            cx = MyMath.iMin(Hmap-step,MapWide-zx);
                            break;
                        case 6: // facing south-east..
                            ...
                            cx = MyMath.iMin(Hmap-step,MapWide-zx);
                            break;
                        case 7: case 8: case 9: // facing south..
                            rx = MyMath.iMin(Vmap-step,MapTall-yx);
                            cx = Hmap-(zx>>1);
                            break;
                        case 10: // facing south-west..
                            ...
                            cx = Hmap+step-zx;
                            break;
                        case 11: case 12: case 13: // facing west..
                            rx = Vmap-(yx>>1);
                            cx = Hmap+step-zx;
                            break;
                    ...
                    break;} //~while // (DoCloseUp) // ZooMapShf cvts c-u(pix) <-> meters
                ...
                if (MyMath.fAbs(VcoFace)>MyMath.fAbs(HsiFace)) { // more N-S than E-W..
                    if (VcoFace<0.0) { // (car aimed north)..
                        ...
                        Bcarx = Hoffm+0.1;} //~if // (aimed north)
                        ...
                        Bcarx = Hoffm+0.1;}} //~if // (aimed south) (more N-S)
                    ...
                    Rcarx = Hoffm+0.1;} //~else // (aimed east, more E-W)
// ZMD=32,32 ZSf=3 ZB=224,642 ZSc=16. ZW=0.2 ZT=8,16
...
//   .. 1,4 8,31 ZT=8,31 23./29. 23./29.
                ...
                for (rx=ImHi-1; rx >= 0; rx += -1) { // fill defaults..................................
                    ...
                    if (rx >= ImHaf) bitz = GrasColo;
                        ...
                        else colo = 0xFFFF;} //~if // aiming for 3399FF in highest sky
                    ...
                    if (dx >= 0) { // meters, close-up res..
                        ...
                        if (optn>1) if (Vat+1.6>Vposn) if (Vat-1.6<Vposn) // optn=opt+shf+lok
                            if (!logy) logy = Log_Draw;} //~if                 // Log_Draw=false
                    if (logy) {
                    ...
                    for (cx=WinWi-1; cx >= 0; cx += -1) {
                        ...
                        if (cx<ImWi) {
                            ...
                            if (rx<kx) { // map shown 1 pix = 1m
                                ...
                                if (zx<MapWy) // zx = (rx>>1)*HalfMap+(zx>>1)+MapIxBase;
                                    if (logy) if (far==0) if (zx>MapWy-4) far = 0x80000000;} //~if
                                ...
                                deep = ZooMapWhLn;} //~if // ZooMapShf cvts c-u(pix) <-> meters
                            else zx = -1;
                            if (logy) if (far==0) {
                                // if ((zx&15)==0) far = 0x80000000; else
                                if (optn>1) if (Vat+1.6>Vposn) if (Vat-1.6<Vposn)
                                    if (Hat+1.6>Hposn) if (Hat-1.6<Hposn) far = 0x80000000;} //~if
                        ...
                        if (thar<myPix.length)
                            myPix[thar] = info;}} //~for //~for (cx) (rx: fill defaults)
                far = 0;
                if (ShowMap) { // add trail of breadcrumbs...................
                    ...
                    for (thar=thar; thar >= 0; thar += -1) {
                        ...
                        if (PeekPixel(rx,cx) != CarColo) // car avatar already there
                            PokePixel(ArtiColo,rx,cx);} //~for // ArtiColo=F90
                    if (ShoTrkTstPts) { // show stay-in-track points as small blue "x"s..
                        if (Left_X != 0) {
                            ...
                            PokePixel(MarinBlue,rx-1,cx+1);
                            PokePixel(MarinBlue,rx+1,cx-1);
                            PokePixel(MarinBlue,rx+1,cx+1);} //~if
                        if (Rite_X != 0) {
                            rx = Rite_X>>16;
                            cx = Rite_X&0xFFF;
                            PokePixel(MarinBlue,rx,cx);
                    ...
                    for (thar=0; thar <= zx; thar += 8) { // some yellow tics..
                        ...
                        PokePixel(0xFFFF00,thar,ImWi);} //~for
                    ...
                    if (info>0) if (PeekPixel(rx,cx)==CarColo) {
                        ...
                        DrawLine(MarinBlue,rx,cx-1,rx,cx+1);}} //~if // (info) (ShowMap)
                if (DrawDash>0) { // DrawDash=12 to draw it
                    ...
                    myDash = myDash + " t=" + LefDash;} //~if // (DrawDash>0)
                                    ...
                                    System.out.println(HandyOps.Dec2Log(aWord,rx,
                                            HandyOps.Dec2Log("/",robe,HandyOps.Int2Log(" ",nBax,
                                                    HandyOps.IffyStr((Qlog&2048) !=0,""," --"
                                                            + HandyOps.ArrayDumpLine(SeenWall,0,24))))) );}} //~if
                        ...
                        if (false) if (InWalls) { // calc shade for distant walls..
                            ...
                            if (DeepDark) {
                                ...
                                if (doit>63) Darken = 0x3F3F3F;      // I don't believe this..
                                    ...
                                    Darken = doit*0x10101;}} //~if
                            Wally = (cx<<24)+CreamWall;} //~if // (InWalls) ..near-white cream
                        thar = rx*WinWi-1;
                            ...
                            if (rx==ImHi-2-DrawDash) { // DrawDash=12
                                if (ZooMapDim !=0) { // draw view trapezoid..
                                    ...
                                    if (Lww>0) if (Rww>0) // MarinBlue=0x0099FF
                                        DrawLine(MarinBlue+Tintx,Lww>>16,Lww&0xFFF,Rww>>16,Rww&0xFFF);} //~if
                                zx = MyMath.Trunc8(Vat+Vat);
                                cx = MyMath.Trunc8(Voffm+Voffm);
                                if (TopCloseView>0) {
                                    if (zx >= TopCloseView) cx = 0;
                                    else if (cx >= TopCloseView) cx = 0;} //~if
                                if (cx>0) DrawLine(MarinBlue+Tintx*2,zx,
                                        MyMath.Trunc8(Hat+Hat)+ImWi+2,cx,
                                        MyMath.Trunc8(Hoffm+Hoffm)+ImWi+2);}} //~if // (DrawDash) (ShowMap)
                        if (VuEdge==0.0) {
                            if (Mtrk !=0) {
                                ...
                                if (VuEdge<0.0) VuEdge = -VuEdge;} //~if // .. = view half-wi, in degs
                            seen = false;} //~if
                        ...
                        if ((cx&15) ==0) for (cx=0; cx <= ImWi-1; cx++) { // .. repeat until off right..
                            ...
                            if (info>0) { // often outside park at first, so..
                                ...
                                if (Log_Draw) if (optn>1) if (step>0) if (step<3) {
                                    aWord = " Or" + rx;
                                    OopsLog = OopsLog + aWord;} //~if // (step)
                                continue;} //~if // (often outside park)
                            ...
                            if (WhitLnSz>0.0) {
                                ...
                                if ((colo&Truk) !=0) {
                                    ...
                                    solidGry = false;} //~if // (GrasColo)
                                else solidGry = false;} //~if
                            else solidGry = false;
                            if (rx==TripLine) { // show TripLine in blue..
                                ...
                                else colo = colo|255;} //~if
                            if (colo <= 0) continue;
                            if (floor==0) floor = rx;
                            if (thar>0) if (myPix != null) if (thar<myPix.length)
                                myPix[thar] = colo; }}} //~for(cx)~while(rx)~for(dx)
                if (!TrakNoPix) if (TrakImages != null) {
                    ...
                    if (yx>0) if (SeenWall != null) {
                        ...
                        if (SeenWall[LayerSz]>0) DoWalls(true);} //~if // --------> "DW}"
                    TmpI = 0;
                    if (NumFax>0) ShoArtifax();} //~if // -------------------> "ShoArt"
                if (Mini_Log) if ((Qlog&0x900) !=0) // Qlog&2048|Qlog&256
        ...
        if (Mini_Log || (why !=0)) { // M_L = true
            ...
            System.out.println(HandyOps.Dec2Log(" (BildFram) ",FrameNo, // why =
                    HandyOps.Dec2Log(myDash,why,OopsLog)));}} //~BuildFrame

    private void InTrackIt(int looky) { // calc new posn & aim to stay in track
        final boolean RightOnly = true;                // (seems OK)
        ...
        while (true) { // find edges (once thru)..
            ...
            if (whom<0) { // on edge, which?
                if ((whom&0x20000000) ==0) { // more H than V..
                    ...
                    ridge = whom;
                    whom = -whom;} //~if // whom>0: starting on right edge
                else ledge = whom;} //~if // whom<0: starting on left edge
                ...
                if (ridge <= 0) { // right edge still unseen..
                    ...
                    if (info==0) {                // = PavColo|Truk = 0x40666666 if inside
                        if ((whom==0) && !doit) { // was/still off-track,
                            ...
                            Hrit = Hat;} //~if
                        else ridge = ridge&0x7FFFFFFF;} //~if // off-track, prior is best
                    ...
                    else ridge = ridge&0x7FFFFFFF;} //~if // prior was best edge
                if (ledge <= 0) { // left edge still unseen..
                    ...
                    if (more==0) {
                        if ((whom==0) && !doit) { // was/still off-track,
                            ...
                            Hlft = Hat;} //~if
                        else ledge = ledge&0x7FFFFFFF;} //~if // off-track, prior is best
                    ...
                    else ledge = ledge&0x7FFFFFFF;} //~if // prior was best edge
                if (doit) if (nx<3) if (fudge <= 0) { // looking fwd, edge unseen..
                    ...
                    if (seen==0) {
                        if ((whom==0) && !doit) { // was/still off-track,
                            ...
                            Hsf = Hat;} //~if
                        else fudge = fudge&0x7FFFFFFF;} //~if // off-track, prior is best
                    ...
                    else fudge = fudge&0x7FFFFFFF;} //~if // prior was best edge
            ...
            if (nx>0) { // can/should center the car in the track..
                ...
                if (RightOnly) { // Vfwd/Hfwd are valid advance..
                    Vat = Vrit+MyMath.Signum(Vlft-Vrit)*2.0;
                    Hat = Hrit+MyMath.Signum(Hlft-Hrit)*2.0;
                    ftmp = Vposn+Vfwd*Velocity;
                    Ztmp = Hposn+Hfwd*Velocity;
                    if (Vfwd>0.0) Vposn = MyMath.fMax(ftmp,Vat);
                    else if (Vfwd<0.0) Vposn = MyMath.fMin(ftmp,Vat);
                    else Vposn = Vat;
                    if (Hfwd>0.0) Hposn = MyMath.fMax(Ztmp,Hat);
                    else if (Hfwd<0.0) Hposn = MyMath.fMin(Ztmp,Hat);
                    else Hposn = Hat;
                    DoRite = true;}
                else {
                    Vat = (Vlft+Vrit)*0.5;
                ...
                else if (!DoRite) if (ftmp+Ztmp>1.5) {
                ...
                if (!DoRite) if (zx>0) { // car shifted, pretend we saw both edges (if close)..
            ...
            if (optn==0) { // why = 3: saw both edges, calculate midline..
                ...
                if (((ledge^ridge)&0x20000000) !=0) { // incompatible sides..
                    ...
                    if (Atmp>Ztmp) { // convert right edge before take average..
                        ...
                        Mconst = (Tcon+Ztmp)*0.5;}} //~if // why = 4 // (used in new aim)
                    ...
                    Mconst = (Tcon+Mconst)*0.5;} //~else // why = 3 // (used in new aim)
                if (ledge !=0) if (ridge !=0) if (((ledge-ridge)&0x7FFFFFFF) !=0)
                    optn = optn+8;} //~if // why = 3/4/5; // else why = 6/7
            ...
            if ((optn&3)==0) { // preferred aim is Horiz..
                ...
                Atmp = MyMath.aTan0(1.0,Mconst);} //~if // Mc>0 is 1st quadrant
            ...
            if ((optn&0x80) !=0) { // ..probably good..
                ...
                else optn = optn|0x40;} //~if
            else optn = optn|0x40;
            if ((optn&0x70)==0) {
                gude = Ztmp; // (one edge) 1st alt if straight NG
                if (DoRite) {
            ...
            break;} //~while // (find edges)
        if (why>1) { // not steering (or already did), but still moving..
            Vat = Vposn;
            Hat = Hposn;
            ftmp = nuly-aim;
            if (DoRite) why = why+256;
            else while (true) { // 2*brk, 1*cont (+rpt, all after log)
                info = 0;
                ...
                if (why<96) { // failed why<64 or didn't have 1st alt, try +sweep..
                    ...
                    why = why-128;
                    doit = false;} //~if
                else nuly = aim;} //~while // keep aim (& fail, but out)
            ValidPixSteps = (Facing==nuly) && ValidPixSteps; // only for fisheye lens
            Facing = nuly;
            why = -why;} //~if
        if (!DoRite) if (dun==8) { // got forward motion, no turn, but maybe should..
            zx = 0;
            nuly = aim;
            nx = PreRite&255;
            ...
            if (nuLft>0) if (nx<SiTwide) if (nuLft<nx) {
                if (nuLft>3) {
                    ...
                    if (zx==3) {
                        ...
                        else if (nuLft<nuRit+2) zx = 0;}} //~if
                else if (zx<8) {
                    nuly = aim+40.0;
                    zx = -2;} //~if
                else if (nuLft<nuRit) zx = -1;
                else if (nuLft==nuRit) zx = 0;} //~if
                ...
                HandyOps.Int2Log("] ",ledge,HandyOps.Int2Log("/",ridge,
                        HandyOps.Flt2Log(" ",Vlft,HandyOps.Flt2Log("/",Hlft,
                                HandyOps.Flt2Log("|",Vrit,HandyOps.Flt2Log("/",Hrit,
                                        HandyOps.Flt2Log(" +",Vstp,HandyOps.Flt2Log("/",Hstp,
                                                HandyOps.Flt2Log(" a=",Atmp,HandyOps.Flt2Log(" z=",Ztmp,
                                                        HandyOps.Flt2Log(" f=",Vfwd,HandyOps.Flt2Log("/",Hfwd,
                                                                aLine))))) ))))) )))));} //~InTrackIt
    ...
    private void MoveCar() { // called once for each frame (not nec'ly sync'sly)
        ...
        while (true) { // no early rtns
            ...
            if (ShowMap) if (nCrumbs==0) {
                ...
                nCrumbs++;} //~if       // MPF = 0.8 = fMinSpeed*fFtime (in pk m/fr)..
            ...
            if ((SpecWheel|prio)==0) { // neither does fSteer..
                ...
                ShoSteer = 0;
                fSteer = 0.0;
                if (nx==0) break;} //~if // why = 2 (stopped)
            ...
            if (RampServos) if (ServoStepRate>0) { // ServoStepRate=0
                ...
                else if (more+ServoStepRate<0) more = -ServoStepRate;} //~if
            ...
            if (SimSpedFixt) {                                      // =FixedSpeed
                ...
                Velocity = doing;} //~if // V is in meters/frame (after physics)
            ...
            if (zx !=0) for (nx=(TapedWheel[0]&255)-1; nx >= 0; nx += -1) {
                ...
                break;} //~for // always exits here, with zx set
            ShoSteer = zx;
            // break;} //~while
            if (!SimSpedFixt) { // (acceleration math is probly wrong)  // !FixedSpeed
                ...
                if (t1<MPF) if (SpecGas<MinESCact) { // stopping..
                    ...
                    if (fSpeed <= 0.1) if (test>0.0) if (test+test<MPF) { // test=abs(oV)
                        ...
                        Velocity = 0.0;
                        fSpeed = 0.0;} //~if
                    if (Velocity==0.0) break;} //~if // why = 774 if stopped, = 6 if already
                if (true) {
                ...
                else doing = (Velocity+oV)*0.5;} //~if // (!SimSpedFixt)
            why = why+3; // why = 7/8/9
            if (OpMode==3) break;
                    ...
                    if (CanSkid<0) if (!Skidding) if (MyMath.fAbs(axel)>RubberTurn) {
                        ...
                        if (Mini_Log) if ((looky==0)||(Qlog<0)) if (Log_AxL !=0) {
                            ...
                            if ((Log_AxL&zx) !=0) {
                                ...
                                Log_AxL = Log_AxL-zx;}}} //~if // (Log_AxL)(Mi_Log)(Skidding)
                    ValidPixSteps = false;} //~if // (t3) VPS: only for fisheye lens
                                ...
                                HandyOps.Dec2Log(" + ",nx,HandyOps.Dec2Log("/",zx,
                                        HandyOps.Flt2Log(" = ",d5*100.0,HandyOps.Flt2Log("c ",axel,
                                                HandyOps.Flt2Log(" >",RubberGrip,
                                                        HandyOps.Flt2Log("/",RubberTurn,""))))) )));} //~if // (Skidding)
                Hposn = d2*doing+Hposn;
                Vposn = Vposn-d3*doing;} //~if // (!SimInTrak)
            ...
            if (ShowMap) {
                ...
                BreadCrumbs[nCrumbs&Crummy] = here; // Crummy = 255
                nCrumbs++;} //~if
            break;} //~while // why = 7/8/9/10/11/12, +16+32+64
                                        ...
                                        HandyOps.Flt2Log("*",fSteer,HandyOps.Flt2Log("*",NormdRad,
                                                HandyOps.Flt2Log("=",x5,HandyOps.Flt2Log(" > ",RubberTurn,
                                                        HandyOps.Flt2Log(" G=",PkGrav,HandyOps.Flt2Log(" fx=",Velocity-oV,
                                                                HandyOps.Flt2Log(" >",RubberGrip,""))))) ))))) );} //~if
                Moved = true;}} //~MoveCar
    ...
    public boolean GetSimFrame(int rose, int colz, byte[] pixels) {
            ...
            try {
                if (OpMode==2) {
                    ...
                    if (NextFrUpdate>0) if (ProcessingTime>0) while (true) {
                        ...
                        else cx = HandyOps.TimeSecs(true);} //~while
                    MoveCar();
                    rx = HandyOps.TimeSecs(true)-cx;
                    ...
                    else NextFrUpdate = NextFrUpdate+FrameTime-ProcessingTime;} //~if
                else if (OpMode==1) MoveCar(); // could crash, so..
                if (NuData>0) {
                    BuildFrame(); // sets NuData=0, FrameNo++, NuData++ if needed VuEdge
                    if (NuData>0) if (FrameNo<4) BuildFrame();} //~if
            } catch (Exception ex) {System.out.println(ex);}
        ...
        for (rx=0; rx <= ImHi-1; rx++) {                   // BayerTile=1 = RG/GB..
            for (cx=0; cx <= WinWi-1; cx++) {
                ...
                if (thar<0) break; if (thar<pixels.length -(Lin2+1)) {
                    pixels[thar+Lin2+1] = (byte)(info&255); // red
                    pixels[thar+Lin2] = aByte;
                    pixels[thar+1] = aByte;
                    pixels[thar] = (byte)((info>>16)&255);} //~if
                thar = thar+2;} //~for
            thar = thar+Lin2;} //~for
        DarkOnce = false;
        return true;} //~GetSimFrame
     ...
     */
    public void DarkFlash() {DarkOnce = true;}
     ...
     */
    public void FreshImage() {NuData++;} // to redraw the scene
     ...
     */
    public boolean StinTrak() {return SimInTrak;}

    /**
     * Returns true if currently in a skid (too much acceleration).
     */
    public boolean InSkid() {return Skidding;}
     ...
     */
    public boolean IsCrashed() {return (OpMode==3);}

    public double ScaleSpeed() {return Velocity*fFPS;} // in m/s park, 8x actual
    public double GetFacing() {return Facing;} // in degrees clockwise from north
    ...
    public int GetMapSize() {
        ...
        return (MyMath.iMin(MapHy,TopCloseView)<<16)+MapWy;} //~GetMapSize
     ...
     */
    public double GetSpeed() {return Speedom;} //~GetSpeed
    ...
    public int TurnRadRow() { // image row+ at turn radius & 2x turn radius
        return t2Radix*0x10000+tRadix;} //~TurnRadRow
    ...
    private void MakeRangeTbl(int why) { // what row+ is far?
        ...
        for (nx=0; nx <= 999; nx++) { // 0 to 75 down, in quarter-degrees
            ...
            if (nx>99) logz = false;
            if (logz) {
                if (aim<1.0) whom = aim*100; else whom = aim;
            ...
            if (tRadix==0) {
                ...
                else if (dx<tx) tRadix = rx+ImHaf-1;} //~if
            if (t2Radix==0) {
                if (dx==t2x) t2Radix = rx+ImHaf;
                else if (dx<t2x) t2Radix = rx+ImHaf-1;} //~if
            while (yx <= xx) { // while (yx+ImHaf <= rx)
                ...
                if (CeilingHi>0.0) if (RowCeiling != null) {
                    ...
                    RowCeiling[yx&ImHmsk] = MyMath.iMax(ImHaf-kx,0);} //~if
            ...
            while (zx>dx) { // dx is in park 6cm units to index this table..
                ...
                zx--;} //~while
            if (zx <= 0) if (yx >= ImHaf) break;} //~for // (only exit) rx valid, >ImHi
        zx = 0;
        ...
        while (RangeRow[zx&2047]==0) {
            ...
            zx++;} //~while
        dx = 0;
        if ((Qlog&256) !=0) if ((Qlog>8)||(why>0))
            System.out.println(HandyOps.Dec2Log(" (MkRngTb/",why,
        ...
        if (tRadix==0) tRadix = ImHi+2;} //~MakeRangeTbl
    ...
    public void Refocus(int newFoc35) { // set new lens focal length (see Zoom35)
        ...
        if (didit) MakeRangeTbl(2);} //~Refocus // also sets tRadix
    ...
    public void LdPebTrak(int kx, int co, int zx, int cx) {
        ...
        while (true) {
            ...
            for (nx=0; nx <= 9; nx++) if ((GotPebz&(1<<nx))==0) // BuildMap '[]' overrides default
            ...
            for (zx=0; zx <= 16; zx += 8) {
                ...
                for (nx=0; nx <= 9; nx++) {
                    ...
                    PebleTrak[nx] = PebleTrak[nx]+(kx<<zx);}} //~for
            why = 0;
            break;} //~while
        ...
        for (nx=0; nx <= 9; nx++) {
            ...
            if ((kx&-8)==0) {
                ...
                aWord = aWord + " 0+0";} //~if
            else aWord = aWord + HandyOps.Colo2Log(" ",kx,"");} //~for   // "LdPebTrk:"
        ValiPavColo();   // MapLogged = D_MapLogged && ((Qlog&1) !=0)..
        if (MapLogged) if ((Qlog&256) !=0) {
            System.out.println(aWord);}} //~LdPebTrak
    ...
    private boolean InitIndex(int whoz, int[] theInx) { // =T if OK, always logs
        ...
        if (theInx != null) if (theInx.length > ArtBase) {
            ...
            if (nx>ArtBase) if (GroundsColors !=0) { // set colors & start pos'n..
                ...
                if (info >= 0) {
                    ...
                    aWord = aWord + HandyOps.Flt2Log("W= ",WhitLnSz," ");} //~if
                PavColo = Color4ix(0);
                PavDk = Color4ix(1);
                GrasColo = Color4ix(2);
                GrasDk = Color4ix(3);
                if (!TrakNoPix) if (NumFax>0) if (TrakImages != null) {
                    ...
                    if (myFax==null) NumFax = 0;
                        ...
                        if (info >= 0) if (info<myFax.length) myFax[info] = 0;}}} //~if
        if (GroundsColors<0) for (nx=nx; nx <= HalfMap*HalfTall-1; nx += HalfMap+1) { // diagonal
            ...
            break;} //~for // (nx)     // otherwise outside background (InWalls=false)
                                                            ...
                                                            HandyOps.Dec2Log(" ",NumFax,HandyOps.Dec2Log(" BG=",nBax,
                                                                    HandyOps.Hex2Log("\n    x",bitz,4,HandyOps.TF2Log(" ",theInx != null,
                                                                            HandyOps.Dec2Log(" ",NumFax,aWord + "\n  ---> "
                                                                                    + HandyOps.ArrayDumpLine(AnimInfo,0,8))))) ))))) ))))) )));} //~if
        ...
        if (nBax>0) if (WalzTall != null) {
            if (Bmap>4) if (Bmap<zx) for (lxx=1; lxx <= 255; lxx++) { // collect wall heights..
                ...
                if (zx+1==0) { // plain wall..
                    ...
                    nBax = lxx;}} //~for // lxx (collect wall heights)
            info = 0;
            if (whom>0) for (zx=whom-1; zx >= 0; zx += -1) { // find most common wall height..
                wide = DidCells[zx&255];
                if (wide>info) info = wide;} //~for
                ...
                for (rx=0; rx <= MapHy-1; rx++) {
                    ...
                    for (cx=0; cx <= HalfMap-1; cx += 4) {
                        ...
                        for (nx=0; nx <= 3; nx++) {
                            ...
                            info = info>>8;}} //~for // (nx)(cx)
                    if (zx>0) aLine = aLine + aWord;
                    zx = 0;} //~for // (rx)
                System.out.println(aLine);}} //~if // (Mini_Log)
        return why >= 0;} //~InitIndex // always true
    ...
    private void ReadTrakIndex() { // only frm LoadTrackIn; see also BuildMap
        ...
        try { while (true) {
            ...
            if (!TrakNoPix) if (tops>0) { // (PatsAcres file has no image component)
                ...
                TrakImages = theInx;}
            why = 0;
            break;} } catch (Exception ex) {why = -why;}
        ...
        if (Qlog<0) System.out.println(HandyOps.Dec2Log("ReadPatsIndx ",xize>>10,
                HandyOps.Dec2Log("K / ",tops>>10,HandyOps.Dec2Log("K (",info,
                        HandyOps.Dec2Log(") = ",why,"")))));} //~ReadTrakIndex
    ...
    private boolean LoadTrackInfo() { // true if success
        ...
        if (UseTexTrak) while (true) { try {
            ...
            for (nx=0; nx <= myMap.length-4; nx++) {
                if (myMap==null) break;
                if (nx<0) break;
                if (nx<myMap.length -3) myMap[nx] = myMap[nx+3];} //~for
        ...
        if (MapIndex==null) {
            ReadTrakIndex();
            if (MapIndex==null) return false;} //~if
        return true;} //~LoadTrackInfo
    ...
    public String toString() {return "TrakSim " + RevDate;}

    public TrakSim() {StartPatty("TrakSimCons");}} //~TrakSim (apw3) (TS)
