 ...
 */
public class TrakSimCamera extends RealCamera {
    ...
    public TrakSimCamera() {
        if (NoisyFaker) System.out.println(HandyOps.Dec2Log("trakSimFiles.SimCamera ", CamHi,
    ...
    public boolean nextFrame(byte[] pixels) { // fills pixels, false if can't (SimCam)
        ...
        return result;
    } //~NextFrame // in trakSimFiles.SimCamera
    ...
    public String toString() { // (SimCam)
        return "trakSimFiles.SimCamera " + errn + ": " + ErrorNumberText(errn);
    }
} //~SimCamera (trakSimFiles) (SC)
